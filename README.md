...
1. **Deep narratives** — each topic is a folder with a `README.md` (math, diagrams, where it shows up in the wild).
2. **Reference code** — Python and/or Rust where we have implemented the idea (e.g. `BLOOM_FILTERS/bloom_filter.py`).
...
|-------|----------------|--------------|
| **Story** | Topic `README.md` → synced to the site | Intuition, formulas, Mermaid diagrams |
...
The long-form “textbook” content stays in **topic folders**. This root README explains **the project**; each `TOPIC_NAME/README.md` explains **that topic**.
...
```mermaid
flowchart TB
  subgraph topics [Topic folders at repo root]
    BF[BLOOM_FILTERS/]
    HLL[HYPERLOGLOG/]
...
- **One folder per topic** — `SCREAMING_SNAKE_CASE` (e.g. `COUNT_MIN_SKETCH/`) with at least a `README.md`.
...
Each topic folder in this repo goes deeper on one slice of that landscape. The site’s [learning path](https://chaitra.pages.dev/) orders a beginner-friendly subset; the [catalog](https://chaitra.pages.dev/catalog) lists everything we track.
...
```
chaitra/
├── BLOOM_FILTERS/          # Example: README + bloom_filter.py + bloom_filter.rs
├── HYPERLOGLOG/
├── GOSSIP_PROTOCOLS/
├── …                       # One directory per topic (README required)
├── website/                # Astro site → Cloudflare Pages
│   ├── src/content/topics/ # Per-slug MDX + synced readme-body.md
│   ├── src/components/     # Labs, catalog, code workbench
│   └── README.md           # Build, deploy, contributor details for the site
...
3. Clone this repo and open the matching folder (e.g. `BLOOM_FILTERS/`) to run Python or Rust locally.

```bash
git clone https://github.com/07AMIT10/chaitra.git
cd chaitra/BLOOM_FILTERS
...
1. Edit or create `YOUR_TOPIC/README.md` at the repo root (and add `.py` / `.rs` if you have reference code).
...
**CI:** pushes touching `website/**` or `BLOOM_FILTERS/**` run the site build via [`.github/workflows/website-ci.yml`](.github/workflows/website-ci.yml).

---

## Topic index (on GitHub)

Every topic with a `README.md` is a subdirectory at the root. Highlights with interactive labs and/or code on the site include:

| Topic folder | Site |
|--------------|------|
| `BLOOM_FILTERS/` | [/topics/bloom-filters](https://chaitra.pages.dev/topics/bloom-filters) |
| `HYPERLOGLOG/` | [/topics/hyperloglog](https://chaitra.pages.dev/topics/hyperloglog) |
| `COUNT_MIN_SKETCH/` | [/topics/count-min-sketch](https://chaitra.pages.dev/topics/count-min-sketch) |
| `CONSISTENT_HASHING/` | [/topics/consistent-hashing](https://chaitra.pages.dev/topics/consistent-hashing) |
| `GOSSIP_PROTOCOLS/` | [/topics/gossip-protocols](https://chaitra.pages.dev/topics/gossip-protocols) |

For the full list and status badges, use the **[catalog](https://chaitra.pages.dev/catalog)** (generated from this repo).
